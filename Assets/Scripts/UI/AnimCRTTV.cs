...
public class AnimCRTTV : MonoBehaviour
{
        ...
        if(this.gameObject.activeSelf)
            for (int i = 0; i < this.transform.childCount; i++)
                if (this.transform.GetChild(i).gameObject.activeSelf)
                    this.transform.GetChild(i).GetComponent<AnimCRTTV>().Open();
    }
    ...
    public void CloseChildren()
    {
        if (this.gameObject.activeSelf)
            for (int i = 0; i < this.transform.childCount; i++)
                if (this.transform.GetChild(i).gameObject.activeSelf)
                    this.transform.GetChild(i).GetComponent<AnimCRTTV>().Close();
    }
    ...
    public void MoveChildren()
    {
        if (this.gameObject.activeSelf)
            for (int i = 0; i < this.transform.childCount; i++)
                if (this.transform.GetChild(i).gameObject.activeSelf)
                    this.transform.GetChild(i).GetComponent<AnimCRTTV>().Move();
    }
